...
HSQREMOTEDBG sq_rdbg_init(HSQUIRRELVM v, unsigned short port, SQBool autoupdate)
{
    ...
    // TODO: why does this allow to open several connections?
    int so_reuseaddr = 1;
    setsockopt(rdbg->_accept, SOL_SOCKET, SO_REUSEADDR, (char *) &so_reuseaddr, sizeof so_reuseaddr);

    // TODO: test in Windows and Linux
#if defined(_WIN32) || defined(_WIN64)
    unsigned long flags = 1;
    ioctlsocket(rdbg->_accept, FIONBIO, &flags);
#else
    int flags = 1;
    if (-1 == (flags = fcntl(rdbg->_accept, F_GETFL, 0))) {
        flags = 0;
    }
    fcntl(rdbg->_accept, F_SETFL, flags | O_NONBLOCK);
#endif
...
{
    if (SQ_FAILED(sq_compilebuffer(rdbg->_v,
                                   serialize_state_nut,
    ...
    while (rdbg->_endpoint == INVALID_SOCKET);
